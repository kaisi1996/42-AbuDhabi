...
/*   Updated: 2023/09/12 13:11:48 by aalkaisi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	put_images(t_data *data)
{
	...
	data->images[5] = mlx_xpm_file_to_image(data->mlx_ptr, 
			"XPM/p_01.xpm", &data->img_width, &data->img_height);
	data->images[6] = mlx_xpm_file_to_image(data->mlx_ptr, 
			"XPM/p_02.xpm", &data->img_width, &data->img_height);
	data->images[7] = mlx_xpm_file_to_image(data->mlx_ptr, 
			"XPM/p_03.xpm", &data->img_width, &data->img_height);
	data->images[8] = mlx_xpm_file_to_image(data->mlx_ptr, 
			"XPM/p_04.xpm", &data->img_width, &data->img_height);
	data->images[9] = mlx_xpm_file_to_image(data->mlx_ptr, 
			"XPM/zombie.xpm", &data->img_width, &data->img_height);
...
int	main(int argc, char **argv)
{
	...
	data.frame = 0;
	data.zombie = 0;
	mlx_string_put(data.mlx_ptr, data.win_ptr, 10, 10, 0x000000FF, "steps: 0");
